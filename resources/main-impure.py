#!/usr/bin/env python3
## Main modified with --impure to handle nixos-user-repository.nix
...
def rebuild_nixos():
    try:
        subprocess.run(["sudo", "nixos-rebuild", "switch", "--impure"], check=True)
    except subprocess.CalledProcessError as e:
...
def update_nixos():
    try:
        ...
        subprocess.run(["sudo", "nixos-rebuild", "switch", "--upgrade", "--impure"], check=True)
    except subprocess.CalledProcessError as e:
